 ...
 * @author Test Valley School
 */
class behaviour_page implements renderable, templatable {
	...
	/**
	 * Earliest date from which to show behaviour points.
	 */
	private $earliest_date;

	/**
	 * Latest date at which to show behaviour points.
	 */
	private $latest_date;
	...
	public function __construct($user, $earliest_date, $latest_date) {
		...
		$this->configuration->username       = $CFG->report_parentprogressview_behaviour_api_user;
		$this->configuration->password       = $CFG->report_parentprogressview_behaviour_api_pass;

		$this->configuration->base           = $CFG->report_parentprogressview_behaviour_api_base;
		$this->configuration->namespace      = $CFG->report_parentprogressview_behaviour_api_namespace;
		$this->configuration->route          = $CFG->report_parentprogressview_behaviour_api_route;
	...
	public function prepare_data($include_form = true) {
		...
		if (is_array($this->attached_usernames) && count($this->attached_usernames) > 0) {
			foreach($this->attached_usernames as $username) {
				...
				$request->add_meta_query('incident_date', $this->earliest_date, '>');
				$request->add_meta_query('incident_date', $this->latest_date, '<');
				...
				$table = new \report_parentprogressview\local\behaviour_table($username, '/report/parentprogressview/behaviour.php');
				$table->setup();
		...
		$data->behaviour_by_pupil = $output;

		$data->featurehelp = get_string('behaviourfeaturehelp', 'report_parentprogressview');
		$data->featurename = get_string('behaviourfeaturename', 'report_parentprogressview');
