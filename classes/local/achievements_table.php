 ...
 * @author Test Valley School
 */
class achievements_table extends \flexible_table {
	...
	public function __construct($uniqueid, $baseurl) {
		...
		$this->set_attribute('class', 'achievements'); 
	...
	public function fill_table_with_json_data($jsondata) {
		if (is_array($jsondata) && count($jsondata) > 0) {
			...
			foreach($jsondata as $key => $item) {
				...
				$row[0] = userdate(strtotime($item->date), get_string('strftimedateshort', 'langconfig'));
				$row[1] = $item->type;
	...
	public function print_nothing_to_display() {
		...
		echo get_string('noachievementsinrange', 'report_parentprogressview');
		echo '</div>';
	}
